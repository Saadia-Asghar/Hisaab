...
export function TransactionItem({ transaction, payerProfile, currentUserId }) {
  const isPayer = currentUserId && payerProfile && currentUserId === payerProfile.id
    ...
    <div className="card flex gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 text-xs text-[var(--text-muted)]">
          {isPayer ? `You paid · split ${transaction.split_count} ways` : `Your share: ${formatRsLabel(transaction.share_paise)}`} · {relativeTime(transaction.created_at)}
        </p>
        <div className="mt-2">
